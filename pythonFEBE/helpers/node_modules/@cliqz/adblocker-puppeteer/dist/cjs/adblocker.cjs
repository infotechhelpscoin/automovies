...
function fromPuppeteerDetails(details) {
    ...
    return adblocker.Request.fromRawDetails({
 ...
 */
class PuppeteerBlocker extends adblocker.FiltersEngine {
    constructor() {
        super(...arguments);
        ...
        this.onFrame = async (frame) => {
            ...
            const parsed = tldtsExperimental.parse(url);
            ...
            const observer = new adblockerContent.DOMMonitor((update) => {
            ...
            do {
                ...
                try {
                    const foundNewFeatures = observer.handleNewFeatures(await frame.$$eval(':root', adblockerContent.extractFeaturesFromDOM));
    ...
    async injectScriptletsIntoFrame(frame, scripts) {
        ...
        if (scripts.length !== 0) {
            for (let i = 0; i < scripts.length; i += 1) {
                promises.push(frame
                    .addScriptTag({
                    content: adblockerContent.autoRemoveScript(scripts[i]),
                })
                    .then(() => {
                    /* Ignore result */
                }));
    ...
    async removeBlockedFrames(frame) {
        ...
        for (const url of await frame.$$eval('iframe[src],iframe[href]', (elements) => elements.map(({ src, href }) => src || href))) {
            const { match } = this.match(adblocker.Request.fromRawDetails({
                url,
                sourceUrl,
                type: 'sub_frame',
            }));
...
Object.keys(adblocker).forEach(function (k) {
    if (k !== 'default' && !Object.prototype.hasOwnProperty.call(exports, k)) Object.defineProperty(exports, k, {
        enumerable: true,
        get: function () { return adblocker[k]; }
    });
});
